...
function LyricsDisplay({ lines, loading, activeLine, elapsed, duration, onSeek, onBack, contrast = 'high', className = '' }: {
  lines: LyricLine[]; loading: boolean; activeLine: number; elapsed: number; duration: number; onSeek: (time: number) => void; onBack?: () => void; contrast?: Settings['lyricsContrast']; className?: string
}) {
  ...
  useEffect(() => {
    ...
    if (!container || !staticLyrics || duration <= 0) return
  ...
  return (
    <div className={`app ${mobilePlayer ? 'mobile-player-open' : ''} ${mobilePlayer && lyricsOpen ? 'lyrics-view' : ''}`} style={{ '--accent': library.settings.accentColor, '--accent-soft': `color-mix(in srgb, ${library.settings.accentColor} 16%, transparent)` } as React.CSSProperties}>
      ...
      <div className={`now-playing ${mobilePlayer ? 'expanded' : ''}`} style={{ '--player-art': library.settings.dynamicBackground && current?.artwork ? `url("${current.artwork}")` : 'none' } as React.CSSProperties}>
        ...
        {mobilePlayer && lyricsOpen && <LyricsDisplay contrast={library.settings.lyricsContrast} className="immersive-lyrics" lines={displayLyrics} loading={lyricsLoading} activeLine={activeLyric} elapsed={elapsed} duration={duration} onSeek={seekTo} onBack={() => setLyricsOpen(false)} />}
...
function SettingsPage({ library, updateInfo, checkingForUpdates, onAddSource, onRemoveSource, onRescan, onUpdateSettings, onCheckForUpdates }: { library: Library; updateInfo: UpdateInfo | null; checkingForUpdates: boolean; onAddSource: () => void; onRemoveSource: (folder: string) => void; onRescan: () => void; onUpdateSettings: (settings: Partial<Settings>) => void; onCheckForUpdates: () => Promise<void> }) {
  const currentVersion = updateInfo?.currentVersion || '1.0.6'
