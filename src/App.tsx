...
function trackQuality(track: Track) {
  if (track.lossless) return `${track.bitDepth || 16}/${Math.round((track.sampleRate || 44100) / 100) / 10}`
  const format = track.format || (track.sourceType === 'local' ? track.path.split('.').pop() : '')
...
function App() {
  ...
  const writablePlaylists = library.playlists.filter((playlist) => playlist.sourceType !== 'jellyfin')
  ...
  const content = () => {
    ...
    if (view === 'playlist') {
      ...
      return <section><div className="feature-heading"><div><p className="eyebrow">Playlist</p><h1>{playlist.name}</h1><p>{playlistTracks.length} songs · {playlist.sourceType === 'jellyfin' ? 'Synced from Jellyfin.' : 'Drag rows to reorder.'}</p></div><div className="heading-actions">{playlist.sourceType !== 'jellyfin' && <button className="secondary-button danger" onClick={() => removePlaylist(playlist.id)}><Trash2 />Delete</button>}<button className="secondary-button" disabled={!playlistTracks.length} onClick={() => shufflePlay(playlistTracks)}><Shuffle />Shuffle play</button><button className="primary-button" disabled={!playlistTracks.length} onClick={() => playlistTracks[0] && playTrack(playlistTracks[0], playlistTracks)}><Play />Play</button></div></div>{playlistTracks.length ? renderRows(playlistTracks, playlist.sourceType === 'jellyfin' ? '' : playlist.id) : <div className="empty-state"><ListMusic /><strong>This playlist is empty</strong><span>{playlist.sourceType === 'jellyfin' ? 'Refresh Jellyfin after adding songs on the server.' : 'Drag songs onto its name in the sidebar or use a song’s menu.'}</span></div>}</section>
    }
    if (selectedAlbum) {
      ...
      return <section className="detail-page" style={{ '--detail-image': lead.artwork ? `url("${lead.artwork}")` : 'none' } as React.CSSProperties}><div className="detail-backdrop" /><div className="detail-hero"><Artwork track={lead} size="large" /><div><p className="eyebrow">Album</p><h1>{lead.album}</h1><button className="artist-link" onClick={() => openArtist(lead)}>{lead.albumArtist}</button><p>{lead.year || 'Unknown year'} · {albumTracks.length} songs · {formatTime(albumTracks.reduce((sum, item) => sum + item.duration, 0))}</p><div className="detail-play-actions"><button className="round-play" aria-label="Play album" onClick={() => playTrack(lead, albumTracks)}><Play /></button><button className="round-play round-play--secondary" aria-label="Shuffle play album" onClick={() => shufflePlay(albumTracks)}><Shuffle /></button></div></div></div>{renderRows(albumTracks)}</section>
    }
    if (selectedArtist) {
    ...
    if (view === 'genres') {
      if (selectedGenre) { const tracks = libraryIndex.tracksByGenre.get(selectedGenre) || []; return <section><div className="feature-heading"><PageHeading eyebrow="Genre" title={selectedGenre} subtitle={`${tracks.length.toLocaleString()} songs`} /><button className="secondary-button" onClick={() => shufflePlay(tracks)}><Shuffle />Shuffle play</button></div>{renderRows(tracks)}</section> }
    ...
    if (view === 'decades') {
      if (selectedDecade) { const tracks = libraryIndex.tracksByDecade.get(selectedDecade) || []; return <section><div className="feature-heading"><PageHeading eyebrow="Decade" title={`${selectedDecade}s`} subtitle={`${tracks.length.toLocaleString()} songs`} /><button className="secondary-button" onClick={() => shufflePlay(tracks)}><Shuffle />Shuffle play</button></div>{renderRows(tracks)}</section> }
    ...
    if (view === 'favorites') { const tracks = library.tracks.filter((track) => library.favorites.includes(track.id)); return <section><div className="feature-heading"><PageHeading eyebrow="Collection" title="Loved Songs" subtitle={`${tracks.length} favorites`} /><button className="secondary-button" disabled={!tracks.length} onClick={() => shufflePlay(tracks)}><Shuffle />Shuffle play</button></div>{renderRows(tracks)}</section> }
    if (view === 'recent') return <section><div className="feature-heading"><PageHeading eyebrow="Listening history" title="Recently Played" subtitle="Pick up where you left off" /><button className="secondary-button" disabled={!historyTracks.length} onClick={() => shufflePlay(historyTracks)}><Shuffle />Shuffle play</button></div>{renderRows(historyTracks)}</section>
    ...
    <div className={`app ${mobilePlayer ? 'mobile-player-open' : ''} ${mobilePlayer && lyricsOpen ? 'lyrics-view' : ''}`} style={{ '--accent': library.settings.accentColor, '--accent-soft': `color-mix(in srgb, ${library.settings.accentColor} 16%, transparent)` } as React.CSSProperties}>
      ...
      <aside className="sidebar">
        <nav>
          ...
          <div className="playlist-nav">{library.playlists.map((playlist) => <button key={playlist.id} className={view === 'playlist' && selectedPlaylist === playlist.id ? 'active' : ''} onDragOver={playlist.sourceType === 'jellyfin' ? undefined : (event) => event.preventDefault()} onDrop={playlist.sourceType === 'jellyfin' ? undefined : (event) => { event.preventDefault(); addTrackToPlaylist(playlist.id, event.dataTransfer.getData('text/polaris-track')) }} onClick={() => { setSelectedPlaylist(playlist.id); showView('playlist') }}><ListMusic /><span>{playlist.name}</span><small>{playlist.trackIds.length}</small></button>)}</div>
...
function SettingsBase({ library, updateInfo, checkingForUpdates, onAddSource, onRemoveSource, onRescan, onUpdateSettings, onCheckForUpdates }: SettingsPageProps) {
  const currentVersion = updateInfo?.currentVersion || '1.0.14'
      ...
      <div className="settings-groups">
        ...
        <div className="settings-group settings-group--release">
          <div className="release-copy">
            <div className="settings-group-title"><ScrollText /><span><h2>What's new in 1.0.14</h2><small>Released through GitHub</small></span></div>
            <ul><li>Jellyfin playlists sync in server order</li><li>Collections have instant shuffle play</li><li>Remote quality labels show the correct format</li></ul>
          </div>
