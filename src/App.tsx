...
function App() {
  ...
  const content = () => {
    ...
    if (view === 'settings') return <SettingsPage library={library} updateInfo={updateInfo} checkingForUpdates={checkingForUpdates} onRescan={rescan} onUpdateSettings={updateSettings} onCheckForUpdates={runUpdateCheck} />
    if (selectedAlbum) {
    ...
    if (selectedArtist) {
      ...
      const globallyPopularLocalTracks = uniqueBy(artistTracks.filter((track) => globalRanks.has(normalizedTitle(track.title))), (track) => normalizedTitle(track.title)).sort((left, right) => (globalRanks.get(normalizedTitle(left.title)) ?? Number.MAX_SAFE_INTEGER) - (globalRanks.get(normalizedTitle(right.title)) ?? Number.MAX_SAFE_INTEGER))
...
function SettingsPage({ library, updateInfo, checkingForUpdates, onRescan, onUpdateSettings, onCheckForUpdates }: { library: Library; updateInfo: UpdateInfo | null; checkingForUpdates: boolean; onRescan: () => void; onUpdateSettings: (settings: Partial<Settings>) => void; onCheckForUpdates: () => Promise<void> }) {
  const currentVersion = updateInfo?.currentVersion || '1.0.3'
